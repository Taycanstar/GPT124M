...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        #key_ query, value projections for all heads, but in a batch
    ...
    def forward(self, x):
        B, T, C = x.size() # batch_size, sequence lnegth, embedding dimensionality (n_embd)
        # calculate query, key, values for all heads in batch and move head forward to the batch
        #nh is the number of heads hs is head size and C is number of channels = nh * hs
        # ef. in gpt2 n_head=12, hs=64, so nh*hs=C=768
        ...
        #attentions (materializes the large (T,T) matrix for all the queries and keys)
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        print(f"number of parameters: {num_decay_params} weight decay, {num_nodecay_params} no weight decay")
...
while x.size(1) < max_length:
    with torch.no_grad():
        ...
        # Ensure no NaNs or infinities in probabilities
